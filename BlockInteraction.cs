...
public class BlockInteraction : MonoBehaviour {
    ...
    void Update () {
        ...
        // Change le type de bloc en fonction de la touche numérique pressée
        ...
        if (Input.GetMouseButtonDown(0) || Input.GetMouseButtonDown(1))
        {
            ...
            // Utilisé pour les clics de souris
            // Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition); 
            // if ( Physics.Raycast (ray,out hit,10)) 
            // {

            // Utilisé pour les viseurs
            if (Physics.Raycast(cam.transform.position, cam.transform.forward, out hit, 4))
            {
                ...
                // Récupère le bloc correspondant dans le monde
                ...
                if (Input.GetMouseButtonDown(0))
                {
                    // Vérifie si la position du bloc correspond à la position du joueur
                    if (b.position != cam.transform.position)
                        update = b.HitBlock();
                }
                else
                {
