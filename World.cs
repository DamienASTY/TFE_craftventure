...
public class World : MonoBehaviour
{
    ...
    IEnumerator BuildRecursiveWorld(int x, int y, int z, int startrad, int rad)
    {
        ...
        // Vérifie les conditions d'arrêt de la génération récursive
        if (rad <= 0 || y < 0 || y > columnHeight) yield break;

        // Vérifie la distance par rapport au joueur
        Vector3 chunkPosition = new Vector3(x * chunkSize, y * chunkSize, z * chunkSize);
        float distanceToPlayer = Vector3.Distance(chunkPosition, player.transform.position);
        if (distanceToPlayer > maxChunkDistance * chunkSize) yield break;

        // Génère les chunks dans les 6 directions (front, back, left, right, up, down)
        // et appelle récursivement la génération sur les chunks adjacents
        BuildChunkAt(x, y, z + 1);
        StartCoroutine(BuildRecursiveWorld(x, y, z + 1, rad, nextrad));
        yield return null;
